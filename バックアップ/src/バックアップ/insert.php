<?php
    ...
    // 貸出リスト (20件)
    // 前半は返却済み(return_dateあり)、後半は貸出中(return_dateがnull)
    $lending_list = [
        // --- 過去の貸出（返却済み） ---
        ...
        // 11. 自校貸出: 学問のすゝめ
        ['school_id' => 1, 'grade' => 6, 'class' => 'A', 'number' => 1, 'book_id' => '913002002', 'lending_date' => '2024-06-01 10:00:00', 'return_date' => null],
        // 12. 自校貸出: 高瀬舟
    ...
    $reservation_list = [
        ...
        // 他校から借りている本は基本的に予約経由
        ['school_id' => 7, 'grade' => 6, 'class' => '1', 'number' => 12, 'book_id' => '933000201', 'status_id' => 3, 'reservation_date' => '2024-04-28 10:00:00'],
        ['school_id' => 8, 'grade' => 5, 'class' => '1', 'number' => 7, 'book_id' => '913000101', 'status_id' => 3, 'reservation_date' => '2024-06-01 11:00:00'],
        ['school_id' => 1, 'grade' => 6, 'class' => 'A', 'number' => 2, 'book_id' => '913001802', 'status_id' => 3, 'reservation_date' => '2024-06-05 10:00:00'],

        // --- キャンセルされた予約 (status_id = 2) ---
        ['school_id' => 2, 'grade' => 6, 'class' => '1', 'number' => 10, 'book_id' => '913000501', 'status_id' => 2, 'reservation_date' => '2024-04-12 09:00:00'],

        // --- 現在進行中の予約 (status_id = 1) ---
        // 5. 貸出待ち: 貸出中(No.14)の「こころ」を待っている
        ['school_id' => 3, 'grade' => 4, 'class' => '2', 'number' => 15, 'book_id' => '913000101', 'status_id' => 1, 'reservation_date' => '2024-06-05 10:00:00'],
        
        // 6. 貸出待ち: 貸出中(No.17)の「羅生門」を待っている
        ['school_id' => 5, 'grade' => 3, 'class' => '1', 'number' => 20, 'book_id' => '913000301', 'status_id' => 1, 'reservation_date' => '2024-06-06 12:00:00'],
        
        // 7. 貸出待ち: 「羅生門」の2人目の待ち
        ['school_id' => 2, 'grade' => 6, 'class' => '1', 'number' => 10, 'book_id' => '913000301', 'status_id' => 1, 'reservation_date' => '2024-06-07 09:00:00'],

        // 8. ★配送中 (Position=0になるケース): 第七小(図書室なし)が、第一小の「風の又三郎」を予約し、移動中
        ['school_id' => 7, 'grade' => 6, 'class' => '1', 'number' => 12, 'book_id' => '913001101', 'status_id' => 1, 'reservation_date' => '2024-06-08 08:30:00'],

        // 9. ★配送中 (Position=0): 第八小が、第二小の「雪国」を予約し、移動中
        ['school_id' => 8, 'grade' => 5, 'class' => '1', 'number' => 7, 'book_id' => '913000701', 'status_id' => 1, 'reservation_date' => '2024-06-07 14:00:00'],

        // 10. 受取待ち: 自校の「1Q84」が確保されカウンターにある
        ['school_id' => 1, 'grade' => 6, 'class' => 'A', 'number' => 1, 'book_id' => '913002601', 'status_id' => 1, 'reservation_date' => '2024-06-01 10:00:00'],

        // 11. 受取待ち: 他校(第九中)から届いた「老人と海」が第十中のカウンターにある
        ['school_id' => 10, 'grade' => 3, 'class' => '2', 'number' => 14, 'book_id' => '933000102', 'status_id' => 1, 'reservation_date' => '2024-06-03 11:00:00'],

        // 12. 貸出待ち: 他校貸出中の本を予約
        ['school_id' => 1, 'grade' => 6, 'class' => 'A', 'number' => 2, 'book_id' => '913002002', 'status_id' => 1, 'reservation_date' => '2024-06-08 10:00:00'],

        // 13. 貸出待ち
        ['school_id' => 9, 'grade' => 2, 'class' => 'A', 'number' => 2, 'book_id' => '933000202', 'status_id' => 1, 'reservation_date' => '2024-06-08 11:00:00']
    ];
    ...
    if (isset($_POST['insert_tmpData'])) {
        ...
        try {
            ...
            //予約テーブルに予約履歴を挿入
            $stmt_reservation = $db->pdo->prepare("INSERT INTO reservation(school_id, grade, class, number, book_id, status_id, reservation_date) VALUES(:school_id, :grade, :class, :number, :book_id, :status_id, :reservation_date)");

            foreach($reservation_list as $reservation) {
                $stmt_reservation->bindValue(':school_id', $reservation['school_id'], PDO::PARAM_INT);
                $stmt_reservation->bindValue(':grade', $reservation['grade'], PDO::PARAM_INT);
                $stmt_reservation->bindValue(':class', $reservation['class'], PDO::PARAM_STR);
                $stmt_reservation->bindValue(':number', $reservation['number'], PDO::PARAM_INT);
            ...
            //貸出テーブルに貸出履歴を挿入
            $stmt_lending = $db->pdo->prepare("INSERT INTO lending(school_id, grade, class, number, book_id, lending_date, return_date) VALUES(:school_id, :grade, :class, :number, :book_id, :lending_date, :return_date)");

            foreach($lending_list as $lending) {
                $stmt_lending->bindValue(':school_id', $lending['school_id'], PDO::PARAM_INT);
                $stmt_lending->bindValue(':grade', $lending['grade'], PDO::PARAM_INT);
                $stmt_lending->bindValue(':class', $lending['class'], PDO::PARAM_STR);
                $stmt_lending->bindValue(':number', $lending['number'], PDO::PARAM_INT);
